...
TODO :
<MUST>
Modify the training loop to train on Imagenet

<COULD>
Split the original Imagenet into train and val set
...
def main():
    ...
    train_loader = create_dataloader(imagenet_dataset,
                                    ...
                                    sampler=None) # Set sampler=None so shuffle=True
    
    num_iter_per_epoch = math.ceil(
                len(imagenet_dataset) / opt['datasets']['train']['batch_size_per_gpu'] )
    total_iters = int(opt['train']['total_iter'])
    total_epochs = math.ceil(total_iters / (num_iter_per_epoch))
